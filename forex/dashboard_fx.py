...
def _summary_row(r: pd.Series, score: float, verdict: str) -> str:
    ...
    carry_cell = (
        f'{r["carry_diff"]:+.2f}%' if pd.notna(r.get("carry_diff")) else "—"
    )
    rr_cell = (
        f'{r["rr_diff"]:+.2f}%' if pd.notna(r.get("rr_diff")) else "—"
    )
    health_cell = (
        f'{r["health_diff"]:+.0f}' if pd.notna(r.get("health_diff")) else "—"
    )
...
def _detail_card(r: pd.Series, score: float, reasons: list[str], verdict: str) -> str:
    ...
    fundamentals_block = f"""
      <b>{html.escape(r['base'])}</b> — y10 {f2(r.get('carry_base'))}%, real {f2(r.get('rr_base'))}%, GDP {f2(r.get('gdp_base'))}%, unemp {f2(r.get('unemp_base'))}%, health {f2(r.get('health_base'))}<br>
      <b>{html.escape(r['quote'])}</b> — y10 {f2(r.get('carry_quote'))}%, real {f2(r.get('rr_quote'))}%, GDP {f2(r.get('gdp_quote'))}%, unemp {f2(r.get('unemp_quote'))}%, health {f2(r.get('health_quote'))}<br><br>
      <b>Δ (base - quote)</b> — carry {f2(r.get('carry_diff'))}%, real {f2(r.get('rr_diff'))}%, GDP {f2(r.get('gdp_diff'))}pp, health {f2(r.get('health_diff'))}
    """
    ...
    <div class="card" style="border-left-color:{color}">
      ...
      <div class="card-grid">
        ...
        <div><h4>FUNDAMENTALS</h4><p>{fundamentals_block}</p></div>
...
<div class="timestamp">Generated {datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")} ·
  {len(df)} pairs &nbsp;·&nbsp; {n_bull} bullish / {n_neut} neutral / {n_bear} bearish</div>
{_macro_block(macro)}
...
<table id="summary"><thead><tr>
  ...
  <th>Carry Δ</th><th>Real Δ</th><th>Health Δ</th><th>Comm Δ</th>
  <th>Vol Regime</th><th>CTA</th><th>LM %ile / Traj</th>
  <th>Verdict</th><th>Score</th>
